function w = adjThreshold(w)
% function w = adjThreshold(w) applies a threshold of L such that any 2
% adjacent rcv locations are at least L apart. 
    [~, inds] = pickSpacing(w.rcvLocs, w.L); 
    w.arrayHeading = w.arrayHeading(inds); 
    w.whaleTime = w.whaleTime(inds); 
    w.rcvLocs = w.rcvLocs(inds, :); 
    w.rcvTime = w.rcvTime(inds); 
    w.trueBearings = w.trueBearings(inds); 
